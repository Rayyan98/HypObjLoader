...
window.onload = function init() {
    ...
    var person = prompt("Please Provide url or relative / absolute path to obj file", "Harry Potter");
    if (person == null || person == "") {
      alert("Default Scene Applied")
      person = "https://raw.githubusercontent.com/Rayyan98/OBJRandom/main/Space%20Station%20Scene.obj"
    }
    ...
    objLoader.load(person, function(object)
    {
        ...
        // var initLength = hyperbolicObject.length
        // for ( var v = 0; v < initLength; v++)
        // {
        //     hyperbolicObject.push(mult(mult(hypRotateXY(cToCDist), hypRotateYZ(cToCDist)), hyperbolicObject[v]))
        // }

        // for ( var v = 0; v < initLength; v++)
        // {
        //     var tmat = mult(hypRotateXY(1.4), mult(hypRotateXY(1.4), hypRotateYZ(1.4)))
        //     hyperbolicObject.push(mult(tmat, hyperbolicObject[v]))
        // }

        // for ( var v = 0; v < initLength; v++)
        // {
        //     hyperbolicObject.push(mult(mult(hypRotateXY(1.4), hypRotateYZ(1.4)), hyperbolicObject[v]))
        // }
        ...
        //colors = []
        for(var i = 0; i < hyperbolicObject.length; i++)
        {
            // var localColor = vec4(Math.random(), Math.random(), Math.random())
            // var localColor = vec4(0.7,1,0.2,1)
            var localColor = vec4(211/255, 211/255, 211/255, 1)
            for(var j = 0; j < 3; j++)
            {
        ...
        // for(var i = 0; i < hyperbolicObject.length / 9; i++)
        // {
        //     // var localColor = vec4(Math.random(), Math.random(), Math.random())
        //     var localColor = vec4(1,1,0,1)
        //     for(var j = 0; j < 3; j++)
        //     {
        //         colors.push(localColor)
        //     }
        ...
        // {
        //     // var localColor = vec4(Math.random(), Math.random(), Math.random())
        //     var localColor = vec4(1,0.5,0.9,1)
        //     for(var j = 0; j < 3; j++)
        //     {
        //         colors.push(localColor)
        //     }
	...
	cToCDist = Math.asinh(Math.sinh(distO) * Math.sin(Math.PI * 2 / polAtVertex / 2)) * 2
	...
	for(var level = 0; level < tilesByLevel.length; level++)
	{
		for(var i = 0; i < tilesByLevel[level].length; i++)
		{
            ...
            for(var v = 0; v < localVs.length; v++)
            {
                ...
                var someV = scale(scaleFactor, groundVert)
                localVs2.push(vec4(someV[0], groundLevel, someV[1], someV[2]))
            ...
            for(var v = 1; v < localVs2.length - 1; v++)
            {
                ...
                localTriangles2.push(vert1, vert2, vert3)
            }

            var walls = []
            for(var v = 1; v < localVs.length; v++)
            {
                var ind1 = v
    ...
    colors = []
    for(var i = 0; i < vertices.length / (22 * 3); i++)
    {
        // var localColor = vec4(Math.random(), Math.random(), Math.random())
        var localColor = vec4(155/255, 118/255, 83/255, 1)
        for(var j = 0; j < (2 * 3); j++)
        {
            colors.push(localColor)
        }
    ...
    currentRotation = hypRotateYW(Math.PI)
...
function ojectMinMax(object_vertices)
{
    ...
    for (var i = 0; i < object_vertices.length; i++)
    {
        ...
        // object_vertices[i][1] -= 2.5 // Trees
        object_vertices[i][1] -= 5 // Space Ship
